...
> **整理定案（2026-04）**：本檔已納入**待辦主表（單一真相）**、**鐵則／SignalBus／目錄互鏈**、**`## 關卡與世界現況（As-built）`** 等結構。**之後不論對話、備份或舊截圖如何寫，凡牽涉架構／關卡／待辦口徑，一律以本檔「現行正文」為準。** 根目錄 **`ARCHITECTURE_legacy_2026-04-04.md`** 僅為整理前**凍結快照**，用於 diff 與歷史對照，**不是**現行規範來源。  
> **補充**：前幾批以**搬運骨架、互鏈、去重**為主；**各條是否已落地**仍須以**程式與主表狀態列**對齊（發現矛盾時**改主表與願景小節**，勿只靠舊段落語感）。
...
**依據**：開頭 **「整理定案（2026-04）」** — 開發與協作以**本檔現狀**為聖經正文；勿以 legacy 快照取代之。  
**程式向**：**鐵則與電台（固定入口）** 表 → 再按需進 **`##` 主標**（見下「章節索引」）。
...
| 目前有哪幾張關、誰是預設 hub、田園在哪一張圖 | **`## 關卡與世界現況（As-built）`**（敘事單一入口）→ 細節 **湖畔**／**Phase 10** |
...
| 你要對齊什麼 | 先讀 |
|--------------|------|
| 需求沒講清能不能開工 | **開工前置作業** → QA |
| UI 能不能直連 `Player`／怪、`SignalBus` 能不能寫業務公式 | **核心五大鐵則** |
| 檔名、`class_name`、訊號命名 | **命名規範** |
| 資料夾、Autoload **職責導讀** | **專案目錄結構**（**註冊權威**：**`project.godot` → `[autoload]`**） |
| 訊號誰 emit／誰聽、Phase 專屬速查 | **SignalBus 規範**（**宣告權威**：**`SignalBus.gd`**） |
| 封印儀式邊界、畫圈與搖桿 | **封印系統協議摘要** ＋ **Phase 9**（`SealUI`／`DialogueHudLocker`） |
| 關卡 hub、預設開局、城鎮／湖畔／家園一句話 | **關卡與世界現況（As-built）** → **湖畔 §8**（拓撲留線）、**待辦主表 B-13** |

### 章節索引（`##` 主標一覽）

1. 開工前置作業（QA） → 2. 核心五大鐵則 → 3. 命名規範 → 4. 專案目錄結構 → 5. SignalBus 規範 → 6. 封印系統協議摘要（**導覽內**另有 **鐵則與電台** 速跳表）  
7. **Phase 4**（寵物轉化、批次建置、**怪物戰鬥／FX 聖經級**、PetManager／寵物 UI／場上寵物；**手動技能小節**為歷史規格草稿，**以 Phase 12 為準**）  
8. **Phase 5**（背包／底欄 HUD）→ 9. **Phase 11**（日記／存檔）→ 10. **Phase 6**（技能特效模板）→ 11. **Phase 7**（頭飾；核心與裝備流程已落地）  
12. **Phase 9**（NPC 對話）→ 13. **關卡與世界現況（As-built）**（城鎮／湖畔／換關敘事入口）→ 14. **湖畔關卡 LakeSideLevel**（場景工程單一真相）→ 15. **Phase 10**（家園採收）→ 16. **Phase 12**（**指揮系統** — **2026-04-05 已落地**，見該章 **§6**）  
17. **待辦主表與追蹤（單一真相）** → 18. 下一階段（新對話起手式；指向主表 ID）→ 19. 願景佇列（長線規格＋ Phase 8 UI 協議）→ 20. 寵物出戰策略 → 21. 開發節奏與總誌同步 → 22. **常見地雷** → 23. AI 溝通詞彙表  

**易重複閱讀區**：**關卡「現在玩得到哪、hub 是誰」**以 **`## 關卡與世界現況（As-built）`** 為敘事入口；**場景樹／碰撞／撒点**仍讀 **湖畔**；**採收／`HomeManager` 檔案表**仍讀 **Phase 10**。**狀態、優先序、是否仍開放**以 **`## 待辦主表與追蹤（單一真相）`** 的 **ID** 列為準；**`## 下一階段`** 只給閱讀順序；**`## 願景佇列`** 給長線規格細節（④→①→③→②）。**指揮系統主線**已結案 — **B-10**／**Phase 12 §6**。**§2.4**：**碰撞後 idle 飄移**已修（2026-04）；**封印／技能後面向／run** — **B-11**。戰鬥／座標細節以 Phase 4「怪物動畫…」為準，**常見地雷**補全域與編輯器坑。
...
4. **資料流（Data flow）**
   - 這件事會新增/更新哪些資料？寫入哪個 `Manager`？透過哪些 `SignalBus` 事件通知？（訊號速查：**SignalBus 規範**；與場景綁定的細節見各 **Phase**「訊號」或「Autoload 與執行流程」。）
...
4. **資料驅動（Data-driven）**
   - 任何可調參數優先放 `Resource(.tres/.res)`（例如 **`resources身分證/monster/`** 之 **`MonsterResource`、`PetResource`** 與對應 `.tres`；`class_name` 定義見同目錄 **`monster_resource.gd`**、**`PetResource.gd`**）
...
> **延伸閱讀（不重複貼規則）**：目錄與 Autoload **職責導讀**見 **專案目錄結構**；訊號**分層與速查**見 **SignalBus 規範**；封印**儀式與結算事件**見 **封印系統協議摘要**。
...
> **Autoload 註冊權威**：單例的**名稱與腳本路徑**以 **`project.godot` → `[autoload]`** 為準。下列為**資料夾與職責導讀**；新增 Autoload 時請**同時**更新專案檔與本節一句說明（避免聖經與建置脫節）。
...
- `src腳本/`：所有邏輯腳本（以功能分類）
  - `autoload管理員/`：全域單例（`SignalBus`、`DataManager`、`GlobalBalance`、`SaveGameManager`、`DiaryManager`、`PetManager`（含 **`party_heal_pending_*`**：多寵補血預約，減同目標溢補）、**`PetCommandManager`**（手動寵物戰技 FSM，見 **Phase 12**）、`InventoryManager`、`ProgressionManager`、`NpcInteractionManager`、`NpcStateManager`、`DialogueManager`、`HomesteadStationDialogue`、`HomeManager`、`PlayerHintCatalog`；**留線**：**`ShopManager`**（NPC 商店：金幣↔道具，見 **湖畔章 §8.4**、**待辦主表 B-13**）— 落地後再註冊 `project.godot`；採收模式／家園關卡切換之業務集中於 `HomeManager`，`SignalBus` 僅宣告對應訊號；情境提示文案集中於 `PlayerHintCatalog`；**單槽存檔**集中於 `SaveGameManager`，**日記／生涯成就**集中於 `DiaryManager`，**玩家／寵物戰鬥經驗分攤**集中於 `ProgressionManager`）
  ...
  - `resources身分證/`：資料藍圖（`.gd` 定義 + `.tres` 資料；含 `dialogue/`、`npc/`、`monster/`、`pet/`、`items/`、`headwear/`、`skill/`、`skill_fx_templates/` 等）
...
> **單一宣告源**：所有 `signal` 定義與簽名以 **`src腳本/autoload管理員/SignalBus.gd`** 為準。  
> **與 Phase 正文**：下列 **Phase 9／10／11** 小節為**速查**（**電台仍無業務邏輯**）；與 **Phase 10「訊號」** 等段落**互鏈**— **增刪訊號**時先改 **`SignalBus.gd`**，再更新本節與對應 Phase（**避免**兩處敘述長期不一致）。
...
### Phase 10 家園／採收（訊號速查；emit／connect 見表）
...
封印屬於「儀式型互動」，請嚴格保持 UI 與世界層分離。（對齊 **五大鐵則**之組件化／Signal-Only UI；啟動 **`seal_mode_toggled`**、結算 **`seal_attempt_finished`** 等見 **`SignalBus.gd`** 與 **SignalBus 規範**之請求／結果分層。）
...
- **影子**：**`scenes場景/entities主角_怪物_寵物/寵物/PetCompanion.tscn`** 使用 `ShadowComponent`；其 `_process` 會自 `AnimatedSprite2D` 同步 `sprite_frames` 並檢查動畫名，避免主體換圖後陰影撥到空字串或不存在動畫。
...
- **`PetResource` 必備視覺**：`.tres` 若只填 `icon`、沒填 `sprite_frames`，場上會透明。封印成功入庫時，若怪物已內嵌 `pet_data` 但缺 `sprite_frames`，`PetManager` 會從當次封印的怪物繼承其 `sprite_frames`。出戰時 `PetCompanion` 再解析：先用 `PetResource.sprite_frames`，缺則載入 `resources身分證/monster/<pet_id>.tres` 取該怪物的 `sprite_frames`；`pet_id` 空或無對應檔時，最後備援 **`resources身分證/monster/slime_green.tres`**。
...
- **場景鐵則**：**`scenes場景/entities主角_怪物_寵物/寵物/PetCompanion.tscn`** 的 **`AnimatedSprite2D` 勿序列化 `animation = …`**（須由 `_apply_pet_resource` 後再 `play`），否則 `instantiate()` 可能報不存在動畫名——見 **常見地雷**「`AnimatedSprite2D` 空／不存在動畫名」。
...
> **訊號互鏈**：**`game_save_requested`／`game_save_finished`** 之 emit／connect 表見 **`## SignalBus 規範` →「日記／單槽存檔（訊號速查；規格見 Phase 11）」**。
...
- **`scenes場景/tools工具/FxPreview.tscn`**：模板總覽與巡覽驗收
- **`scenes場景/tools工具/fx_authoring/`**（例：**`warning_circle.tscn`**、**`fire.tscn`**）：可在 2D 視圖直接調參
...
- 量產節奏：**`fx_authoring/`** 母場景精修 → **`FrameBakeTool`** 烘焙 → **`SpriteFrames`** 接回技能

---

## Phase 7：頭飾系統（**核心與裝備流程已落地**；逐角錨點／幀級補正與美術迭代仍可能繼續）
...
- **主角（單一角色，腳本 + 節點）**
  ...
  - **`scenes場景/entities主角_怪物_寵物/主角/Player.tscn`**：`AccessoryPoint`（`Marker2D`，僅供錨點座標）；**`AccessorySprite` 為 `Player` 直接子節點**，場景樹排在 **`AnimatedSprite2D` 之後**，`_update_accessory_anchor` 內 **`accessory_sprite.position = accessory_point.position`** 對齊頭部
- **相容策略**
...
- **頭飾與身體、前景樹的 z／排序（2026-03-29 後定版）**  
  ...
  - **驗收／易誤踩（場景序列化）**：**`Player.tscn`** 存檔時**勿**寫入根節點 **`y_sort_enabled = true`**、**勿**給 **`AccessorySprite`** 單獨 **`z_index` 相對 +1**（Inspector／複製節點時常不小心打開，會復發「身體在樹後、頭飾在樹前」）。要讓頭飾蓋住身體請只動**同父節點下的順序**，勿用 z 硬抬。寵物／怪場景同樣避免「為了蓋身體」隨意加頭飾 z。  
  - 寵物／怪：**`scenes場景/entities主角_怪物_寵物/寵物/PetCompanion.tscn`**、**`scenes場景/entities主角_怪物_寵物/怪物/MonsterBase.tscn`** 同樣 **`AccessorySprite` 為根下獨立節點**（非掛在 `AccessoryPoint` 下），並在各自 `_update_accessory_anchor` 同步 `accessory_sprite.position = accessory_point.position`。
- 背包頭飾頁已可單擊裝備；若有出戰寵物，會先選「裝給主角 / 裝給出戰寵物」
- **`resources身分證/headwear/slime_hat.tres`** 已作為開發期預設頭飾注入背包（替代 NPC/寶箱尚未完成流程）
...
- 戶外關（湖畔／城鎮）**戴頭飾**繞前景樹走一圈：頭飾與身體對樹冠的**前後遮擋須一致**（若頭飾單獨「壓在樹上」，先復查 **`Player.tscn`** 是否又序列化根 **`y_sort_enabled`** 或 **`AccessorySprite` 多加 z；規則見上節「z／排序」與「驗收／易誤踩」）

---

## Phase 9：NPC 對話／互動（MVP 地基已落地）

> **狀態**：資料流與 Manager／`SignalBus` 邊界已依「純訊號 UI」落地；主場景預設關卡為**城鎮**（**`FallenLeafTown`**）。**對話框帳簿風視覺、左右欄對齊**已落地（見 **`DialogueLedgerButtonStyle`、`DialoguePanel`** 與下方「實作補記」）。**`npc_*`／`dialogue_*` 等訊號名稱速查**：**`## SignalBus 規範` →「Phase 9 NPC／對話（訊號速查）」**（與本節「Autoload 與執行流程」並讀）。  
...
**預設開局、城鎮／湖畔／家園語意、換關與 hub**— 見 **`## 關卡與世界現況（As-built）`**。本節只列與 **Phase 9** 除錯最相關的提醒：

- **`LakeSideLevel`**：**`ForegroundDecor`＋`ForegroundCanopyHoist`** 執行期改掛 **`level_container`**，與主角同層比 Y；完整編排見 **`## 湖畔關卡`**。
- **`FallenLeafTown`**：**`LeafZones`＋`TownLeafAmbientVfx`**；田園 **`HomesteadBundle`** 與 **Phase 10** 銜接。
- **`Main.tscn`**：**`LevelContainer` 子關卡**＋**`Player`**＋**`PetCompanionSpawner`**；排序鐵則見 Phase 10「層級規則」。**勿**對 **`Player`** 覆寫 **`y_sort_enabled = true`**（Phase 7）。
...
3. **湖畔關卡無怪物**  
   - **現況（2026-03-29）**：`LakeSideLevel` 已內建 **`ScatteredSlimes`**（10 隻綠史萊姆，`MarkersPropSpawner` + **`resources身分證/monster/slime_green.tres`**）與 **`ScatteredRocks`**（5 顆石）；封印驗收應有場上目標。若仍 **`fail_and_reset`**，請查圈內是否無 `monsters` 群組、或目標已死亡／離場。
...
## 關卡與世界現況（As-built；敘事單一入口）

> **用途**：用**一段話＋小節**對齊「repo 裡目前有哪幾張可玩關、hub／田園在哪、換關怎麼走」。**場景樹、碰撞多邊形、撒点、`MarkersPropSpawner`、怪物奧義、儀式 UI 細節**仍以 **`## 湖畔關卡（LakeSideLevel）…`** 與 **`## Phase 10`** 檔案表為工程單一真相。企劃向**四圖拓撲**、洞窟／Boss／`ShopManager`／Boss 後片尾等**留線**見 **湖畔 §8**～**§8.6** 與 **待辦主表 B-13**。

### 主場景與換關（已落地）

- **`Main.tscn`**：**`LevelContainer`** 裝載當前關卡子實例；**`Player`**、**`PetCompanionSpawner`** 與關內可比 Y 實體須符合 **Phase 10**「層級規則」— **勿**對 `Player` 根亂開 **`y_sort_enabled`**（頭飾見 Phase 7）。
- **`HomeManager`**：**`switch_to_lake`／`switch_to_town`／`switch_to_homestead`**（及 `*_async`）；**`LevelTransitionOverlay`** 電影淡、**`request_area_title`** 區域名、**`PlayerController.snap_camera_after_warp()`**（詳 **湖畔 §5**、Phase 10「實作補記（2026-04）」）。

### 落葉鎮（城鎮）

- **場景**：**`scenes場景/levels/town/FallenLeafTown.tscn`**；底圖 **`hometown_01.png`**（`assets圖片_字體_音效/環境/城鎮含家園/`）。
- **現況**：**`LevelContainer` 預設子關卡**＝**城鎮**（與 **Phase 9「關卡與主場景」**一致）；**右上田園區**內 **`HomesteadBundle`**＝主線**種田／駐寵／採收**（`HomesteadZone`、`HomesteadCrop`、**`HomeManager`** — 與 **Phase 10** 同語意）；**`Art/Ambience`＋`LeafZones`／`TownLeafAmbientVfx`** 落葉粒子。
- **傳送**：與**湖畔**等之 portal 錨點在場景內；設計意圖（進城落點在進田園前）見 **湖畔 §8.1**。

### 湖畔（史萊姆湖畔／野戰）

- **場景**：**`scenes場景/levels/lake_side/LakeSideLevel.tscn`**；大地圖、`TerrainCollision`、`MarkersPropSpawner`（石／史萊姆）、**`ForegroundDecor`＋`ForegroundCanopyHoist`**、環境 **寶寶鳥**兩實例（**湖畔 §4c**）。
- **語意**：**野戰／封印試煉**；關內 **`HomesteadBundle`／試作田**僅**過渡**，**主線家園以城鎮田園為準**（**湖畔 §8.2**）。

### 獨立家園關（演練／備用）

- **`scenes場景/levels/homestead/HomesteadLevel.tscn`**：整張家園、無傳送門；可用於驗收；主線敘事以**城鎮內嵌田園**為主。

### 世界拓撲與未實作（規格；非 As-built）

- **企劃定調**（城鎮 hub、湖畔、洞窟、Boss、感謝名單／騎虎歸途、`ShopManager`、死亡復活醒於家園等）：**表格與敘述**見 **湖畔 §8**～**§8.6**；執行狀態見 **待辦主表 B-13**。
- **預設關卡與存檔**：企劃長期以**城鎮**為 hub；**`Main.tscn` → `LevelContainer` 預設子關卡**以**場景實例為準**（**現行**＝**`FallenLeafTown`**，與 **Phase 9「關卡與主場景」**、本節「落葉鎮」一致）；**讀檔落地關**以 **`SaveGameManager`** 為準。若未來改預設關或進場狀態，一併改 **`HomeManager`** 與 **§8.1** 區域名協議。

---

## 湖畔關卡（LakeSideLevel）地圖與場景編排（實作定版 2026-03-29；**世界分線補記 2026-04**）

> **段落目標**（工程細節）：**湖畔關**之場景樹、碰撞、層級、撒点、怪物奧義、換關儀式感— **單一真相**在本章。**一句話現況與 hub 語意**先讀 **`## 關卡與世界現況（As-built）`**；**四圖拓撲與長線留線**在本章 **§8～§8.6**。

### 1. 場景樹摘要（`scenes場景/levels/lake_side/LakeSideLevel.tscn`）
...
| **`ScatteredRocks`／`ScatteredSlimes`** | 皆掛 **`MarkersPropSpawner.gd`**；子節點僅 **`Marker2D`**；`prop_scene` 分別為 **`scenes場景/entities主角_怪物_寵物/environment環境物件/Rock.tscn`**、**`scenes場景/entities主角_怪物_寵物/怪物/MonsterBase.tscn`**；史萊姆另設 **`prop_data`**＝**`resources身分證/monster/slime_green.tres`**（**`add_child` 前**寫入 `MonsterBase.data`）。 |
| **`BabyBirdPerches`／`LakeBabyBird`／`LakeBabyBird2`** | 環境寶寶鳥：**`BabyBirdPerches`** 下 **`Marker2D`** 群組 **`lake_baby_bird_perch`**（離屏再生挑點）；關卡根下 **兩** 個 **`scenes場景/entities主角_怪物_寵物/怪物/BabyBirdMonster.tscn` 實例**，各 **`lake_ambient_save_slot`** 0／1。詳 **§4c**。 |
...
- **關卡摆法（現行定版）**：**`scenes場景/levels/lake_side/LakeSideLevel.tscn`** 根下**直接**放 **兩** 個 **`BabyBirdMonster` 實例**（`LakeBabyBird`／`LakeBabyBird2`），各設 **`lake_ambient_save_slot = 0`／`1`**（與 **`GlobalBalance.LAKE_AMBIENT_BABY_BIRD_TOTAL`** 一致）；`data` 可留空，執行期由腳本補 **`resources身分證/monster/baby_bird_monster.tres`**。**亦可**改用 **`MarkersPropSpawner`** 指到 **`scenes場景/entities主角_怪物_寵物/怪物/BabyBirdMonster.tscn`** 並覆寫 **`prop_data`**，但須**逐一**指定不重複的 **`lake_ambient_save_slot`** 並同步 **`LAKE_AMBIENT_BABY_BIRD_TOTAL`**。
...
- **獨立場景檔**：**不要**在 `LakeSideLevel` 內再疊一張完整家園底圖；**長期**以 **落葉鎮關卡場景**（或 **`TownLevel`／`FallenLeafTown.tscn` 等**）+ **`LevelContainer` 換子實例**為主。`HomeManager` 已保留 **`switch_to_homestead`／`switch_to_lake`／`switch_to_town`**（**`Main` 預設關**＝城鎮）；**日後**對稱擴充 **`switch_to_cave`／`switch_to_cave_boss`** 等，見 **§8**。**`HomesteadLevel.tscn`** 可續作純家園演練場，**主線**以**城鎮內嵌田園**為準。
...
|-----------|------------------------|------------|
| **1 城鎮** | **落葉鎮**（**`scenes場景/levels/town/FallenLeafTown.tscn`**；底圖 **`hometown_01.png`**） | **右上**＝**田園家園**（種田、駐寵、採收；**複製 Phase 10 田園子系統**至此圖）。**左下**＝通往 **湖畔** 的傳送點（或等價「出城門」區）。 |
| **2 湖畔** | **`scenes場景/levels/lake_side/LakeSideLevel.tscn`** | 野戰：史萊姆、環境寶寶鳥等。**上方**＝通往 **洞窟** 入口。 |
...
- **主線出生／預設關卡**：企劃以**城鎮**為 hub；**`Main.tscn` → `LevelContainer` 預設子關卡**＝**落葉鎮**（**`scenes場景/levels/town/FallenLeafTown.tscn`**；細節見 **As-built**「落葉鎮」）。**讀檔落地關**以 **`SaveGameManager`** 為準；調整預設關或進場狀態時同步 **`HomeManager`** 與區域名。**洞窟／Boss 等新關**仍屬 **B-13** 留線。
...
### 9. 家園寵物行為體規格草案（P0；**待辦主表 B-01／B-02**）
...
> **關卡敘事**：**哪張圖有主線田園、預設 hub、換關語意**— 先讀 **`## 關卡與世界現況（As-built）`**。  
> **第一階（2026-03-28 結案）**：採收模式、HUD／搖桿互斥、滑掃成熟作物入包、區域浮字、封印鈕在家園隱藏、**`LevelContainer` 同層排序**；**世界提示**管線— `player_world_hint_changed`＋`PlayerHintCatalog`＋**`HarvestModeHint`**。  
> **2026-04**（細節見「實作補記（2026-04）」）：成熟株進園提醒、拖曳教學（**`ProgressionManager`**，掃 **2 株**）、**`HarvestWorldDim`／作物提亮**、採收中 NPC 近距提示規則、**電影淡換關＋區域名**。**洞窟／Boss／商店**與長線拓撲見 **湖畔 §8**、**待辦主表 B-13**。
...
| 關卡場景 | `scenes場景/levels/lake_side/LakeSideLevel.tscn` | **敘事語意**見 **`## 關卡與世界現況（As-built）`** → **湖畔**。**`Background` 大地圖**、`TerrainCollision`、`HomesteadBundle`（**`HomesteadZone`／`Crops`／`SoilPlots`**；主線田園以**城鎮**為準，見 **§8.2**）、**`MarkersPropSpawner`**、**`ForegroundDecor`＋`ForegroundCanopyHoist`**；工程細節見 **`## 湖畔關卡`** |
| 關卡場景 | `scenes場景/levels/homestead/HomesteadLevel.tscn` | **敘事**見 **As-built**「獨立家園關」；`HomesteadZone` 全圖；無傳送門 |
| 關卡場景 | `scenes場景/levels/town/FallenLeafTown.tscn` | **敘事**見 **As-built**「落葉鎮」；城鎮底圖、田園 **`HomesteadBundle`**、傳送、**`TownLeafAmbientVfx`**；**`LevelContainer` 預設子關卡**（以場景為準） |
...
- **其他寵物**：亦可**駐留家園**（展示或日後加成）；**`PetUI`「放置家園」按鈕尚未製作**（**待辦主表 B-15**）——規格：**平時反灰**，**僅當主角人身處家園場景時可點**（由 **`player_in_homestead_changed(in_homestead: bool)`** 類訊號驅動 UI，不直連 `Player` 節點）。
...
### 訊號（`SignalBus.gd`；與全域速查互鏈）

> **`emit`／`connect` 對照表**（採收模式、區域名、成熟株採收入庫等）：**`## SignalBus 規範` →「Phase 10 家園／採收（訊號速查；emit／connect 見表）」**。**新增或改名訊號**時以 **`SignalBus.gd`** 為準，並**同步**更新該表與本節補充說明。

- **payload／教學語意**（表內僅略述者）：**`player_world_hint_changed`** — **無 payload 必 `emit(..., null)`**；Dictionary 可含 **`instant_text`**、**`hold_sec`／`fade_out_sec`**，或打字序列鍵（見 **`HarvestModeHint`** 註解）。**家園採收（2026-04）**：進家園有成熟株→**每趟進入**一次 **`instant` 提醒**；採收中→**第一次流程**拖曳教學（**`ProgressionManager`**，掃 **2 株**後關）；採光→單句收工＋關採收；離開家園 `emit("", false, null)` 收起。
- **駐留／入庫語意**：`pet_homestead_station_requested`、`pet_home_roster_changed`；採收結算仍走 **`item_collected`**／**`request_effect_collect`**（見上表）；背包 UI 可監聽 **`inventory_changed`**（或日後更細結果訊號）。
...
### 留線（給下一輪對話／日誌）
- **狀態／優先序**：以 **`## 待辦主表與追蹤（單一真相）`**（**B-01**～**B-18**）為準；本小節保留關鍵詞與上下文。
- **關鍵詞**：`Phase10 slice2`、`作物時間軸`、`嘟嘟`、`翻土`、`PetUI 放置家園`、`HomeManager 存檔欄位`、`DiaryUI`、`HarvestSwipeCapture` 底欄留白、`diary_ui_close_requested`。  
- **湖畔地圖段落**：**已定版**（2026-03-29）— **敘事入口** **`## 關卡與世界現況（As-built）`**；工程細節 **`## 湖畔關卡（LakeSideLevel）地圖與場景編排（實作定版）`**；**2026-04 補記**：**§8** **四圖**、**傳送點在進家園前**、`hometown_01.png`、**城鎮複製田園**、**`ShopManager`**、**死亡復活家園醒來**、**§8.6 Boss 後劇情／感謝名單（可略過）／騎虎歸途至家園與妹妹**；湖畔內測試田園**淘汰**。**落葉鎮**：**換關電影淡**、**區域名（湖畔↔鎮）**、**落葉粒子**、**採收體驗 polish** 已落地（見 Phase 10 表＋**實作補記 2026-04**）。下一工作包：**洞窟／Boss 場景**、**過場與感謝名單演出**、**商店與金幣消費**、其餘場景 **動畫與粒子 polish**；**換場與儀式感 UI（電影淡＋標題）** 主線**已補強**（該章 **§5**）。
...
> 下列為當期對話落地、聖經原「待做／願景」欄位尚未逐條改寫者之**快照**；**是否仍開放／優先序**以 **`## 待辦主表與追蹤`** 為準；細節以程式與 `docs/` 為準。

- **家園 Phase 10 延伸（程式仍保留；場景曾切換以利試採收）**  
  - **`HomesteadSoilPlot`**（**`scenes場景/levels/homestead/HomesteadSoilPlot.tscn`** + **`src腳本/entities/homestead/HomesteadSoilPlot.gd`**）、**`HomesteadCrop`**：`class_name`、`free_after_pickup`／`harvest_recycled`；**`SkillResource.is_homestead_till_skill`**、**`resources身分證/skill/skill_homestead_till.tres`**；**`GlobalBalance.HOMESTEAD_CROP_GROW_SEC`**；**`HomeManager.request_homestead_hints_refresh`**；**`PetCompanion`** 在家園內對 **`homestead_soil_plot`** 翻土。  
  - **獨立寵物「嘟嘟」**：**`resources身分證/pet/dudu_pet.tres`** + **`resources身分證/pet/dudu_sprite_frames.tres`**（動畫名對齊史萊姆集；圖用占位）；**非**封印轉化必經路；接入指南 **`docs/嘟嘟動畫接入指南.md`**。史萊姆 **`resources身分證/pet/slime_green_pet.tres`** 僅保留治療技能（無翻土）。  
  - **場景現況（2026-04 對齊 `.tscn`）**：**`FallenLeafTown`**、**`LakeSideLevel`** 之 **`HomesteadBundle/Crops`** 為 **8** 株預設成熟 **`HomesteadCrop`**（`MatureCrop01`～`08`）；**`HomesteadBundle/SoilPlots`** 各掛 **4** 個 **`HomesteadSoilPlot`**（`SoilPlot01`～`04`）。**`HomesteadLevel`** 根下 **`Crops`**／**`SoilPlots`** 同為 **8** 株＋**4** 土格（無 **`HomesteadBundle`** 包一層）。美術改場景後以**實際節點**為準；土格歷史與還原見 **`docs/家園翻土土格暫時移除與還原.md`**。  
- **湖畔氛圍**：**`LakeSideAmbientVfx.gd`** 螢火蟲 **`CPUParticles2D`** 的 **`scale_amount_min/max`** 已調為 **1.0～2.5**（基礎變體仍為隨機縮放）。  
- **落葉鎮氛圍（2026-04）**：**`TownLeafAmbientVfx.gd`**＋**`scenes場景/levels/town/FallenLeafTown.tscn` → `Art/Ambience/LeafZones`**：每樹區 **兩發射器** 兩色、**`color_ramp` 漸顯漸隱**；粒子 **scale** 刻意大於螢火蟲 **1.0～2.5**；**換關電影淡**見上 **湖畔 §5** 與 **`LevelTransitionOverlay`**。  
...
- **主角頭飾預設**：**`scenes場景/entities主角_怪物_寵物/主角/Player.tscn`** **不**再序列化 **`equipped_headwear`**（避免與背包唯一裝備衝突）；開局帽仍可由 **`InventoryManager.STARTER_HEADWEAR_PATHS`** 入包。**`PlayerController`**：`equipped_headwear` 匯出列於 **「頭飾錨點」** 群組（緊接 `Head Anchor Offset`），避免收合在「頭飾位置」子群組內找不到。
...
> **注意**：看家寵已升級為 **`HomesteadPetStationAgent` 巡遊 agent**（idle／抽點移動、active／light 分層、翻土優先、存檔快照等，見下列 bullets）；**避障品質／大規模壓測／節奏 polish** 仍以 **待辦主表 B-01／B-02** 為準。

#### 已落地（程式與資源）

- **經濟與擊殺獎勵（資料驅動）**  
  - **`MonsterResource`**：`gold_reward`、`xp_reward`（例：**`resources身分證/monster/slime_green.tres`**）。  
...
- **家園駐留與種子佇列（資料＋UI）**  
  ...
  - **本輪進一步（P0 起手）**：`HomesteadPetStationAgent` 已由站位代理升級為 **`CharacterBody2D` 巡遊 agent**（`idle(0.8~2.2s) -> choose_target -> move -> idle`），並接入：  
    ...
    5) **看家陰影**（**`scenes場景/entities主角_怪物_寵物/homestead/HomesteadPetStationAgent.tscn`** 掛 **`ShadowComponent`**）。  
...
- **寶寶鳥（2026-04 已落地；美術／數值可持續迭代）**  
  - **寵物**：**`resources身分證/pet/baby_bird_pet.tres`** + **`resources身分證/pet/baby_bird_sprite_frames.tres`**（專用圖與動畫軌）。  
  - **環境怪**：**`resources身分證/monster/baby_bird_monster.tres`** + **`scenes場景/entities主角_怪物_寵物/怪物/BabyBirdMonster.tscn`**／**`AmbientBabyBirdMonster.gd`**（見上 **湖畔 §4c**）。  
...
- **幸運（最小可用）**
  ...
  - **完整玩法**（封印成功率等技能向堆疊）：**延後至下一批「寵物技能細修」**，與 **`luck_bonus_rate`** 規則一併收斂（**待辦主表 B-07**）。
- **測試用種子**  
  - 成熟株 **`item_template`** 與 **`resources身分證/items/homestead_crop_demo.tres`** 為同一試作品種（**`is_seed = true`**）。**`DEBUG_SEED_TEST_ITEMS`** 開啟時 **`InventoryManager`** 會 **`grant_item_stack_by_id("homestead_crop_demo", 10)`**（新局／無存檔種子路徑）。

#### 已知問題（更新）

- **家園行為體已進入 P0 第一版**：可巡遊、可分層、可翻土判斷，但仍缺「更完整避障／導航」與大規模壓測下的細節 polish — **狀態見待辦主表 B-01**。  
- **下一輪核心方向**：在現有 agent 基礎上補齊建築密集區避障品質、巡遊觀感（轉向／停留節奏）、以及嘟嘟翻土與種子佇列的完整閉環驗收 — **B-02**。

---

## 待辦主表與追蹤（單一真相；2026-03-31 晚間盤點起；**2026-04-05 Phase 12 主線已結案**；**2026-04-12 Phase 12 盤查入聖經**；**2026-04-04 首批：待辦／下一階段／願景狀態合併**）

> **維護規則**：**狀態**（進行中／未實作／已結案／延後）**只在本表更新**。各 Phase、湖畔章若再寫到同一題，請改為 **「見主表 B-xx」** 或一句摘要，避免多處各改各的。實作維持資料驅動與 `SignalBus` 鐵則。

| ID | 項目 | 類型 | 狀態 | 主要錨點 | 備註 |
|----|------|------|------|----------|------|
| B-01 | 家園寵物數十隻上限＋巡遊行為體（非站樁） | P0 | **進行中** | 湖畔 **§9**、Phase 10 實作補記、已知問題 | active／light 分層＋存檔已有；待避障品質、壓測、節奏 polish |
| B-02 | 嘟嘟家園翻土（常駐邏輯） | P0 | **進行中** | Phase 10、`is_homestead_till_skill` | 待種子佇列、土格演出、完整驗收閉環 |
| B-03 | 各怪物 `.tres` 金幣／XP 填齊 | Backlog | 待查 | Phase 4、`MonsterResource.gold_reward`／`xp_reward` | 綠史萊姆等已設；哥布林／蘑菇等逐檔對表 |
| B-04 | 進化完整系統＋等級節奏 | 願景② | 未實作（入口已留） | 願景 **②**、`pet_evolution_requested` | 資料／流程／UI 未接；存檔欄位見願景② |
| B-05 | 皇冠／王者史萊姆正式版 | Backlog | 進行中 | Phase 10 實作補記「原型資源」 | 占位 `.tres`；美術／數值／技能 |
| B-06 | 寶寶鳥（環境怪＋飛行寵物跟隨） | — | **已結案（2026-04）** | 湖畔 **§4c**、Phase 4 飛行類 | 兩實例＋分槽存檔、`AmbientBabyBirdMonster`；美術可 polish |
| B-07 | 幸運完整玩法（封印成功率等被動鏈） | Backlog | **延後** | `luck_bonus_rate`、採收最小版 | 併入下一批寵物技能細修 |
| B-08 | 環境生物可封印（非戰鬥；願景①） | 願景① | **MVP 已落地（2026-04）** | **B-06**、湖畔 **§4c**、`AmbientBabyBirdMonster`、`participates_in_combat` | **已驗**：寶寶鳥首種—可封印、不入戰鬥鎖敵、分槽存檔、轉 **寵物**。**仍非「完整玩法」**：多品種（蝴蝶等）、採集／封印**被動光環全鏈**、Manager 修飾器表—見願景 **①** 末段與 **B-07** |
| B-09 | 生蛋冷卻對話 | Backlog | 未實作 | — | — |
| B-10 | 指揮系統（手動寵物戰技，右側兩鈕） | — | **已結案（2026-04-05）** | **Phase 12** §6 | 歷史齒輪草稿：**Phase 4** 手動技能小節；**§2.4** 飄移已修，剩餘 **B-11** |
| B-11 | 戰鬥體感：封印／技能後面向與 run | Backlog | 待排 | **Phase 12** §2.4 | **勿**與指揮 FSM 同捆 |
| B-12 | 湖畔：水／花／螢火蟲／火把等動畫與粒子 | Polish | 可排 | 湖畔 **§5**「仍可排入…」 | 非阻塞；`EffectManager`／場景內節點 |
| B-13 | 世界線：洞窟／Boss 場景、`ShopManager` 消費、死亡復活家園醒來、Boss 後感謝名單／騎虎歸途 | 世界線 | 留線 | 湖畔 **§6～§8.6**、Phase 10「留線」 | **`Main` 預設關**＝落葉鎮見 **§8.1** 與 **As-built**；`HomesteadLevel`／城鎮田園見 **§6／§8.2** |
| B-14 | Phase 9 延伸：任務／多型效果、多 NPC 仲裁深化 | Backlog | 部份已落地 | **Phase 9** | 匯出、條件欄位、`REQUEST_QUEST` 等已落地；其餘隨本列更新收口 |
| B-15 | Phase 10 切片：作物時間軸／再生、`PetUI`「放置家園」鈕 | Backlog | 待做 | Phase 10「建議最小落地順序」2～4 | 與 **B-01**／**B-02** 交錯時以 P0 為先 |
| B-16 | `pet_mount_requested`／坐騎綁定 | 可選 | 未實作 | — | 可選擴充 |
| B-17 | NPC 與寵物世界演出互動 | 願景④ | 待做 | 願景 **④**、`DialogueManager` | 對話關閉驅動 NPC 動畫等 |
| B-18 | 地圖邊緣指引 UI | 願景③ | 待做 | 願景 **③** | 與小地圖／`player_world_hint` 單一真相 |

---

## 下一階段（新對話起手式；細項見主表 ID）

> **不重複貼待辦全文**：**狀態**以 **`## 待辦主表與追蹤（單一真相）`** 為準。下列僅**閱讀順序**與**章節鏈結**；實作維持 Signal-Only UI 與本文件鐵則。

1. **P0**：**B-01**、**B-02** — 規格 **湖畔 §9**；索引 **Phase 10**（建議最小落地順序、實作補記 2026-03-31、已知問題）。  
2. **世界線**：**B-13** — **湖畔 §8** 為敘事主軸；商店／洞窟／Boss 後演出見 **§8.4～§8.6**；田園複製與湖畔試作田淘汰 **§8.2**。  
3. **戰鬥體感剩餘**：**B-11** — **Phase 12 §2.4**（與 **B-10** 已結案主線分案）。  
4. **關卡 polish**：**B-12** — **湖畔 §5**（與 **B-13** 可並行排期）。  
5. **Phase 9／10 其餘**：**B-14**、**B-15**、**B-03**、**B-05**、**B-07**、**B-09** — 見各 Phase 正文。  
6. **長線願景（規格細節）**：**B-17**（④）、**B-18**（③）、**B-04**（②）；**①** 之 **MVP** 已落地（**B-06／B-08**），**①** 章內其餘為**擴充規格**— 順序仍記 **④→①→③→②**，見 **`## 願景佇列`** 各 `###`。  
7. **可選**：**B-16**。

**已結案／不必再當「下一步」主線**：**B-10**（**Phase 12 §6**）；日記／單槽存檔 **Phase 11**；Phase 8 視覺主線（2026-03）— **Phase 5／7／8** 與願景內「Phase 8 UI 視覺風格協議」。**湖畔大地圖與換關儀式**已定版 — **湖畔** 整章；**儀式感 UI 待修隊列**已結案（**湖畔 §5**）。

**Phase 9 已落地速記**（細節仍讀 Phase 9）：`DialogueChoiceEntry` 條件欄位、`DialogueManager` 過濾、`DialogueEffectEntry.REQUEST_QUEST` + `dialogue_quest_requested`、`DialogueGraphResource.export_table_rows`／`export_table_tsv`；**`NpcStateManager` 好感**與 **`SaveGameManager`**；**`career_milestone_id` → `DiaryManager`**。

**Phase 10 已落地速記**：作物三階段、離線成長、成熟掃描上限 10、`HomeManager` 存檔分語意、駐留相關訊號宣告、看家可見可互動；**世界提示**請沿用 `player_world_hint_changed`＋payload（見 Phase 10「留線」）；**放生**語意見 Phase 5 — **`SignalBus.gd` 禁止寫業務邏輯**。

---

## 願景佇列（長線規格；**實作順序 ④ → ① → ③ → ②**）

> **與主表分工**：本節為**規格與禁忌**的單一真相；**狀態**以 **`## 待辦主表與追蹤`** 為準（**B-17**、**B-18**、**B-04**；**B-08**＝願景① **MVP 已落地**，章內長文仍含**未落地的擴充句**，讀時對照主表）。共通鐵則：**Signal-Only UI**、**`SignalBus.gd` 不寫業務公式**；資料驅動優先，**禁止** `if pet_id == "某某"` 特例鏈。  
> **已結案例外**：指揮戰技 — **B-10**／**Phase 12**（§1／§1.1／§5／§6）。**§2.4** 飄移已修；**封印／技能後動畫** — **B-11**。
...
> **現況（2026-04）— 與主表 B-06／B-08 對齊**：**湖畔環境寶寶鳥**已落地— **`participates_in_combat = false`**、**`sealable_entity`**、專用 **`AmbientBabyBirdMonster`**、封印結算與 **分槽存檔**、轉 **`baby_bird` 寵物**與飛行跟隨（**湖畔 §4c**、**Phase 4**）。下列條列自 **「建議身分」** 起多為**擴充規格**（第二種以後環境生物、被動數值全鏈）；**勿**整段當作「尚未做」— **首種管線已通**。

- **目標（願景／擴充）**：蝴蝶／昆蟲／小鳥等**像環境裝飾**，仍可被**封印圈收服**；**不參與近戰鎖敵與怪 AI 追逐**；被捕捉後統一語意為**療癒／好運**— 加成 **採集率、封印成功率**（數值堆疊規則另表；**完整被動鏈**見 **B-07**／幸運玩法，**非**本節已承諾落地）。
...
### 與 `Monster_DevLog_v4.xlsx` 同步規則（固定）
...
- **本專案已處理範例**：**`scenes場景/entities主角_怪物_寵物/怪物/MonsterBase.tscn`** **根下** **`AccessorySprite`**（與 `AccessoryPoint` 分開）曾序列化 **`animation = &""`**——這會直接觸發 `set_animation("")`；與「開場約 1 秒內**雙**錯誤」吻合（常另有一個子節點同類問題）。**修正方式為刪除該行**，勿在 Inspector 把 Animation 清成空白後存檔。本體 `AnimatedSprite2D` 在資料驅動下通常仍有 **`idle_down`**；若除錯訊息寫的是 **`idle_down`** 但實際成因是空字串，請仍以全文搜尋 **`animation = &""`** 為準。**主角 `scenes場景/entities主角_怪物_寵物/主角/Player.tscn` 的 `AccessorySprite`**：若**未**序列化 `sprite_frames`（改由執行期 `equipped_headwear` 指派），**勿**在場景裡留 **`animation = &"idle_side"`** 等名——否則載入時 C++ 會報 **`There is no animation with name 'idle_side'`**；刪除該行或確保圖集內含該動畫名。
