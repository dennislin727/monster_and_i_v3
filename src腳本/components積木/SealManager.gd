...
func execute_sword_fall():
	Engine.time_scale = 1.0 # 恢復正常速率
	SignalBus.seal_ui_requested.emit(false) # 濾鏡與 UI 演員瞬間消失
	
	if world_player:
		world_player.show()
		world_player.set_physics_process(true)
		world_player.is_seal_mode = true # 進入世界按壓模式
	
	if target_monster:
		# 文字提示：長壓幫助封印!!
		SignalBus.popup_text.emit(target_monster.global_position + Vector2(0, -90), "長壓幫助封印!!", Color.YELLOW)
		# 啟動怪物的真實壓制法陣
...
func is_touching_monster(screen_pos: Vector2) -> bool:
	...
	return target_monster.global_position.distance_to(world_pos) < 100
