...
## 與 `PlayerMoveState` run／idle 門檻（velocity.length() > 10）對齊；無輸入時清掉摩擦漸減與碰撞滑移殘速，避免 idle 仍漂移
const IDLE_VELOCITY_SNAP_LENGTH_SQ := 100.0
...
## 手動寵物地面技鬆手後：短暫鎖 `spell_` 姿態，避免 MoveState 立刻蓋回 run/idle。
var _pet_command_pose_left: float = 0.0
...
func _physics_process(_delta: float) -> void:
	if Engine.is_editor_hint():
		...
		return
	if _pet_command_pose_left > 0.0:
		_pet_command_pose_left = maxf(0.0, _pet_command_pose_left - _delta)
	...
	move_and_slide()
	if input_dir == Vector2.ZERO and velocity.length_squared() <= IDLE_VELOCITY_SNAP_LENGTH_SQ:
		velocity = Vector2.ZERO
...
## Phase 12：瞄準地面戰技鬆手確認後，朝落點方向播 `seal_`（與封印長壓法陣同款解析）。
func play_pet_command_seal_pose(target_world: Vector2) -> void:
	if Engine.is_editor_hint():
		return
	if anim_sprite == null or anim_sprite.sprite_frames == null:
		return
	var to_t := target_world - global_position
	var face := last_direction
	if to_t.length_squared() > 4.0:
		face = to_t.normalized()
		last_direction = face
	var seal_anim := resolve_directional_animation_name("seal_", face)
	if seal_anim.is_empty():
		return
	if not _safe_animated_play(anim_sprite, seal_anim):
		return
	var dur := _get_body_anim_duration_sec(seal_anim)
	_pet_command_pose_left = dur if dur > 0.05 else 0.45


func _get_body_anim_duration_sec(anim_name: StringName) -> float:
	...
	var sp := sf.get_animation_speed(anim_name)
	if sp > 0.0:
		sum /= sp
	return sum
...
## 換關傳送後呼叫：立刻對齊 Camera2D 平滑目標，避免黑幕揭開時相機「飄回」。
func snap_camera_after_warp() -> void:
	if Engine.is_editor_hint():
		return
