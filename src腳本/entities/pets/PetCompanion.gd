...
## 戰技鈕遮罩分母：與本次實際寫入的 `_combat_skill_cd` 同步，避免與 `PetManager` 快取公式微小差異造成進度先跑完。
var _cd_bar_total_combat: float = 0.0
## 同上（補血手動／自動觸發時寫入 `_heal_cd`）。
var _cd_bar_total_heal: float = 0.0
var _pet_aoe_delay_left: float = 0.0
var _pet_aoe_skill_pending: SkillResource = null
## 落地圈已在 `_begin_pet_line_aoe` 生成時為 true；`effect_manager` 缺失時改由 `_fire_pet_line_aoe` 補發。
var _pet_ground_telegraph_spawned_at_begin: bool = false
## 手動地面技：結算時改用此世界座標（一次消費）。
var _manual_aoe_impact_valid: bool = false
var _manual_aoe_impact_world: Vector2 = Vector2.ZERO
...
## 跟隨／戰鬥站樁時 lerp 殘速與碰撞滑移：與主角 idle 門檻同量級
const STATIONARY_VELOCITY_SNAP_LENGTH_SQ := 100.0
...
func _snap_stationary_velocity_after_slide() -> void:
	if _anim_lock > 0.0 or _heal_casting:
		return
	if _motion_intent_vel.length_squared() > 36.0:
		return
	if _combat_active():
		if _dist_to_follow_slot > PET_ATTACK_RANGE * 0.92:
			return
	else:
		if _dist_to_follow_slot > FOLLOW_ARRIVE_DISTANCE + 14.0:
			return
	if velocity.length_squared() <= STATIONARY_VELOCITY_SNAP_LENGTH_SQ:
		velocity = Vector2.ZERO
...
func _resolve_scaled_cooldown_for_owned_skill(sk: SkillResource) -> float:
	if sk == null:
		return 0.0
	if _data == null:
		return sk.cooldown
	var lv := 1
	for e in _data.skills:
		if e != null and e.skill == sk:
			lv = maxi(1, e.skill_level)
			break
	return GlobalBalance.resolve_pet_skill_cooldown_scaled(sk.cooldown, lv)


func _resolve_scaled_heal_cooldown() -> float:
	if _data == null:
		return GlobalBalance.PET_HEAL_COOLDOWN
	var base: float = _data.heal_cooldown if _data.heal_cooldown > 0.0 else GlobalBalance.PET_HEAL_COOLDOWN
	var lv := 1
	for e in _data.skills:
		if e != null and e.skill != null and e.skill.type == SkillResource.SkillType.HEAL:
			lv = maxi(1, e.skill_level)
			break
	return GlobalBalance.resolve_pet_skill_cooldown_scaled(base, lv)


## 槽 0 手動戰技鈕：剩餘冷卻（秒）；`skill` 須與 `PetCommandManager.cached_manual_skill` 一致。
func get_manual_skill_cooldown_remaining(skill: SkillResource) -> float:
	if party_slot_index != 0 or skill == null:
		return 0.0
	match skill.type:
		SkillResource.SkillType.HEAL:
			return maxf(0.0, _heal_cd)
		SkillResource.SkillType.AOE_ATTACK:
			return maxf(0.0, _combat_skill_cd)
		_:
			return 0.0


## 剩餘冷卻 >0 時，進度條分母（秒）；與 `get_manual_skill_cooldown_remaining` 同源快照。
func get_manual_skill_cooldown_bar_denominator(skill: SkillResource) -> float:
	if party_slot_index != 0 or skill == null:
		return 0.0
	var rem := get_manual_skill_cooldown_remaining(skill)
	if rem <= 0.0001:
		return 0.0
	match skill.type:
		SkillResource.SkillType.HEAL:
			return maxf(maxf(_cd_bar_total_heal, rem), 1e-6)
		SkillResource.SkillType.AOE_ATTACK:
			return maxf(maxf(_cd_bar_total_combat, rem), 1e-6)
		_:
			return maxf(rem, 1e-6)
...
func _physics_process(delta: float) -> void:
	...
	_try_homestead_till()
	move_and_slide()
	_snap_stationary_velocity_after_slide()
...
func _reset_breadcrumb_trail() -> void:
	...
	_cd_bar_total_combat = 0.0
	_cd_bar_total_heal = 0.0
	_pet_aoe_delay_left = 0.0
...
func _begin_pet_line_aoe(facing_target_world: Vector2, manual_impact_world: Variant = null) -> void:
	var sk := _combat_aoe_skill
	if sk == null:
		return
	if manual_impact_world is Vector2:
		_manual_aoe_impact_valid = true
		_manual_aoe_impact_world = manual_impact_world as Vector2
	else:
		_manual_aoe_impact_valid = false
	_combat_skill_cd = _resolve_scaled_cooldown_for_owned_skill(sk)
	_cd_bar_total_combat = _combat_skill_cd
	_pet_aoe_skill_pending = sk
	_pet_aoe_delay_left = sk.trigger_delay
	var an := sk.animation_name.strip_edges()
	if an.is_empty():
		an = "spell"
	_play_pet_animation(an, facing_target_world - global_position)
	var dur := _get_anim_duration_sec(StringName(an))
	if dur <= 0.0:
		dur = 0.55
	_lock_anim_for(maxf(dur, sk.trigger_delay) + 0.2)
	# 落地圈與怪物一致：動畫開播當下即顯示警示（勿拖到 trigger_delay 結束才生成）
	_pet_ground_telegraph_spawned_at_begin = false
	var em_early := get_tree().get_first_node_in_group("effect_manager") if get_tree() else null
	if sk.aoe_use_ground_target and em_early != null and em_early.has_method("play_ground_slam_aoe_from_skill"):
		var impact_early := facing_target_world
		if _manual_aoe_impact_valid:
			impact_early = _manual_aoe_impact_world
		em_early.play_ground_slam_aoe_from_skill(sk, self, false, impact_early)
		_pet_ground_telegraph_spawned_at_begin = true
...
func _fire_pet_line_aoe() -> void:
	...
	if em == null:
		return
	if sk.aoe_use_ground_target:
		if not _pet_ground_telegraph_spawned_at_begin and em.has_method("play_ground_slam_aoe_from_skill"):
			var impact_fb := _combat_target_pos()
			if _manual_aoe_impact_valid:
				impact_fb = _manual_aoe_impact_world
			em.play_ground_slam_aoe_from_skill(sk, self, false, impact_fb)
		_pet_ground_telegraph_spawned_at_begin = false
		if _manual_aoe_impact_valid:
			_manual_aoe_impact_valid = false
		return
...
func _try_heal(delta: float) -> void:
	...
	if target_h == null:
		return
	_heal_cd = _resolve_scaled_heal_cooldown()
	_cd_bar_total_heal = _heal_cd
...
## Phase 12：編隊槽 1 手動戰技（由 PetCommandManager 呼叫）；`world_pos` 僅地面圈 AOE 有意義。
func execute_manual_command(skill: SkillResource, world_pos: Vector2) -> bool:
	if party_slot_index != 0:
		return false
	if not is_instance_valid(self) or health.current_hp <= 0 or _celebrating:
		return false
	if skill == null:
		return false
	match skill.type:
		SkillResource.SkillType.HEAL:
			return try_manual_heal_command()
		SkillResource.SkillType.AOE_ATTACK:
			if skill.aoe_use_ground_target:
				return try_manual_ground_aoe_at(world_pos)
			return try_manual_line_aoe_command()
		_:
			return false


func try_manual_heal_command() -> bool:
	if _heal_casting or _heal_cd > 0.0:
		return false
	if _anim_lock > 0.0:
		return false
	var amt: int = _data.heal_amount if _data and _data.heal_amount > 0 else GlobalBalance.PET_HEAL_AMOUNT
...
func try_manual_ground_aoe_at(impact_world: Vector2) -> bool:
	if _combat_aoe_skill == null:
		return false
	if _pet_aoe_delay_left > 0.0 or _combat_skill_cd > 0.0 or _anim_lock > 0.0 or _heal_casting:
		return false
	if not _combat_aoe_skill.aoe_use_ground_target:
		return false
	_begin_pet_line_aoe(impact_world, impact_world)
	return true


func try_manual_line_aoe_command() -> bool:
	if _combat_aoe_skill == null:
		return false
	if _pet_aoe_delay_left > 0.0 or _combat_skill_cd > 0.0 or _anim_lock > 0.0 or _heal_casting:
		return false
	if _combat_aoe_skill.aoe_use_ground_target:
		return false
	var tp := _combat_target_pos() if _combat_target != null else global_position + Vector2.RIGHT * 40.0
	_begin_pet_line_aoe(tp, null)
	return true
