...
func _physics_process(delta: float) -> void:
	...
	move_and_slide()
	# `Idle` 未每幀設 velocity；`Chase` 貼身等 CD 時亦會 velocity=0 + idle 動畫 — 碰撞滑移後殘速同樣會緩漂
	if state_machine and state_machine.current_state:
		var sn := state_machine.current_state.name
		if sn == &"Idle":
			velocity = Vector2.ZERO
		elif sn == &"Chase" and anim and velocity.length_squared() <= 100.0:
			var an := String(anim.animation).to_lower()
			if an.contains("idle"):
				velocity = Vector2.ZERO
