...
## 舊版預設開局在湖畔；讀檔時改為城鎮床邊（與 FallenLeafTown `PlayerSpawn_Bed` 對齊）。
const _TOWN_LEVEL_SAVE_PATH := "res://scenes場景/levels/town/FallenLeafTown.tscn"
const _MIGRATE_LAKE_TO_TOWN_POS := Vector2(820.0, -505.0)
...
func _apply_all(data: Dictionary) -> void:
	...
	if _should_migrate_lake_save_to_town(want_level):
		want_level = _TOWN_LEVEL_SAVE_PATH
		ppos = _MIGRATE_LAKE_TO_TOWN_POS
	if not want_level.is_empty():
		...
		if cur != want_level:
			...
			elif want_level.contains("FallenLeafTown"):
				await HomeManager.switch_to_town_async()
			else:
...
func _should_migrate_lake_save_to_town(level_path: String) -> bool:
	var p := level_path.strip_edges()
	if p.is_empty():
		return false
	return p.contains("LakeSideLevel")
