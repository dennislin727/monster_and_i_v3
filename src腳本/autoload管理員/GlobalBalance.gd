...
## 換關全螢幕漸黑／漸透（HomeManager → LevelTransitionOverlay）
const LEVEL_TRANSITION_FADE_OUT_SEC := 0.38
const LEVEL_TRANSITION_FADE_IN_SEC := 0.42
...
## 寵物 `PetSkillEntry.skill_level`：每升一等，冷卻乘以 (1 - 此係數)；Lv1 = `base_cooldown`。
const PET_SKILL_COOLDOWN_REDUCTION_PER_LEVEL := 0.07


func resolve_pet_skill_cooldown_scaled(base_cooldown: float, skill_level: int) -> float:
	var b: float = maxf(0.05, base_cooldown)
	var lv: int = maxi(1, skill_level)
	var mult: float = 1.0 - float(lv - 1) * PET_SKILL_COOLDOWN_REDUCTION_PER_LEVEL
	mult = clampf(mult, 0.38, 1.0)
	return maxf(0.15, b * mult)
